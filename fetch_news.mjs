...
async function main() {
  ...
  if (!targetLink) {
    ...
    if (isAuto) {
      ...
      if (!target) {
        ...
        console.log(`✅ 找到早间新闻: ${target.title}`);
  ...
  try {
    ...
    const articleId = createArticle({
      date: dateInfo.dateStr,
      title: `【谢站长之家】${dateInfo.dateStr}（${dateInfo.weekday}）三分钟新闻早餐`,
      content: plainText,
